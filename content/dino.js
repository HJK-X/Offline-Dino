...
if (!old_url.includes("chrome://")) {
    window.history.pushState("", "", "/content/dino.html/" + old_url);
}
var url = document.getElementById("url");
url.href = old_url;
url.text = old_url;

var settingsUrl = document.getElementById("settings");
settingsUrl.href = "/content/settings.html?" + old_url;
...
async function load() {
    ...
    var sprites = document.getElementById("offline-resources-sprites");
    sprites.src = offline.SPRITES;

    document.getElementById("offline-sound-press").src = offline.PRESS;
    document.getElementById("offline-sound-hit").src = offline.HIT;
...
function Runner() {
    ...
    this.outerContainerEl = document.getElementById("main-frame-error");
    this.containerEl = document.getElementById("application");
...
Obstacle.prototype = {
    ...
    getGap: function (gapCoefficient, speed) {
        ...
        var maxGap = Math.round(
            minGap * Runner.config.OBSTACLE_MAX_GAP_COEFFICIENT
        );
...
Trex.prototype = {
    ...
    setBlinkDelay: function () {
        this.blinkDelay = Math.ceil(Math.random() * Runner.config.BLINK_TIMING);
    },
    ...
    startJump: function (speed) {
        if (!this.jumping) {
            ...
            this.jumpVelocity =
                Runner.config.INITIAL_JUMP_VELOCITY - speed / 10;
            this.jumping = true;
            this.reachedMinHeight = false;
            this.speedDrop = false;
        }
    ...
    endJump: function () {
        if (
            ...
            this.jumpVelocity < Runner.config.DROP_VELOCITY
        ) {
            this.jumpVelocity = Runner.config.DROP_VELOCITY;
        }
    },
    ...
    updateJump: function (deltaTime, speed) {
        ...
        if (this.speedDrop) {
            this.yPos += Math.round(
                this.jumpVelocity *
                    Runner.config.SPEED_DROP_COEFFICIENT *
                    framesElapsed
            );
        } else {
        ...
        this.jumpVelocity += Runner.config.GRAVITY * framesElapsed;
    ...
    update: function (deltaTime, distance) {
        ...
        if (!this.acheivement) {
            ...
            if (distance > 0) {
                ...
                if (distance % Runner.config.ACHIEVEMENT_DISTANCE == 0) {
            ...
            if (this.flashIterations <= Runner.config.FLASH_ITERATIONS) {
                this.flashTimer += deltaTime;

                if (this.flashTimer < Runner.config.FLASH_DURATION) {
                    paint = false;
                } else if (this.flashTimer > Runner.config.FLASH_DURATION * 2) {
                    this.flashTimer = 0;
...
NightMode.prototype = {
    ...
    update: function (activated, delta) {
        ...
        if (activated && (this.opacity < 1 || this.opacity == 0)) {
            this.opacity += Runner.config.NIGHTMODE_FADE_SPEED;
        ...
        if (this.opacity > 0) {
            this.xPos = this.updateXPos(this.xPos, Runner.config.MOON_SPEED);

            // Update stars.
            if (this.drawStars) {
                for (var i = 0; i < Runner.config.NUM_STARS; i++) {
                    this.stars[i].x = this.updateXPos(
                        this.stars[i].x,
                        Runner.config.STAR_SPEED
                    );
                }
            }
            this.draw();
    ...
    draw: function () {
        ...
        starSize *= 2;
        starSourceX = Runner.spriteDefinition.STAR.x;
    ...
    placeStars: function () {
        var segmentSize = Math.round(
            this.containerWidth / Runner.config.NUM_STARS
        );
        ...
        for (var i = 0; i < Runner.config.NUM_STARS; i++) {
            ...
            this.stars[i].sourceY =
                Runner.spriteDefinition.STAR.y +
...
function Horizon(canvas, spritePos, dimensions, gapCoefficient) {
    ...
    // Cloud
    this.clouds = [];
    this.cloudSpeed = Runner.config.BG_CLOUD_SPEED;
...
Horizon.prototype = {
    ...
    updateClouds: function (deltaTime, speed) {
        ...
        if (numClouds) {
            ...
            if (
                numClouds < Runner.config.MAX_CLOUDS &&
